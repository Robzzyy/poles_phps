...
function connexion(){
    return new PDO("mysql:host=localhost;dbname=voiture", "root", "");
}
 ...
 */
function selectionId(string $table, int $nom_marque): ?array
{
    $requete = connexion()->query("SELECT * FROM $table
                                   WHERE nom_marque = $nom_marque");
    if( $requete ) {
...
/**
 * @return bool si un abonné est connecté et qu'il a 
 *               le niveau administrateur, la fonction retourne true
 */
function connecteAdmin(){
    $admin = connecte();
    if($admin){
        return $admin["niveau"] == NIVEAU_ADMIN;
    }
    return false;
}